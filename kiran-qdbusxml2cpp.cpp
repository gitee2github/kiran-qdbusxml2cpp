...
#define PROGRAMNAME "kiran-qdbusxml2cpp"
...
static QString propertyNotifier(const QDBusIntrospection::Property &property)
{
    ...
    notifier = property.name + "Changed";
    return notifier;
}
...
static void writeProxy(const QString &filename, const QDBusIntrospection::Interfaces &interfaces)
{
    ...
    for (const QDBusIntrospection::Interface *interface : interfaces)
    {
        ...
        hs << "public:" << endl
           ...
           << "    ~" << className << "();" << endl
           << endl;
        QString connectToPropertyChanged = QString(R"(    const_cast<QDBusConnection&>(connection).connect(service,path,"org.freedesktop.DBus.Properties","PropertiesChanged","sa{sv}as",this,SLOT(%1(QDBusMessage)));)").arg(HANDLE_DBUS_PROPERTY_CHANGED_SLOT_NAME);
        ...
        for (const auto property : interface->properties) {
            ...
            if (emitChangedSignalValue == "false")
            {
                continue;
            }
            QString name = property.name;
            QByteArray type = qtTypeName(property.type, property.annotations);
            cs << "    if (propertyName == QStringLiteral(\"" << property.name << "\"))" << endl;
            cs << "    {" << endl;
            cs << "        const " << type << " &" << property.name << "Value" << " = qvariant_cast<" << type << ">(value);" << endl;
            cs << "        Q_EMIT ptr->" << name << "Changed(" << property.name << "Value);" << endl;
            cs << "        return;" << endl;
            cs << "    }" << endl;
            cs << endl;
        }
        cs << "    qWarning() << \"property not handle: \" << propertyName;" << endl;
        cs << "    return;" << endl;
        cs << "}" << endl
           << endl;

        // handle dbus property changed
        cs << "void " << className << "::" << HANDLE_DBUS_PROPERTY_CHANGED_SLOT_NAME << "(const QDBusMessage &message)" << endl
           << "{\n"
        ...
        for (const QDBusIntrospection::Property &property : interface->properties)
        {
            ...
            QString notifierSignalDeclare = QString("void %1(%2 value) const;").arg(notifier, constRefType);
            hs << "    " << notifierSignalDeclare << endl;
        }
        hs << endl;

        hs << "private Q_SLOTS: //PRIVATE PROPERTY CHANGED SLOT" << endl;
        QString dbusPropertyChangedSlot = QString("    void %1(const QDBusMessage &message);").arg(HANDLE_DBUS_PROPERTY_CHANGED_SLOT_NAME);
        hs << dbusPropertyChangedSlot << endl;
...
static void writeAdaptor(const QString &filename, const QDBusIntrospection::Interfaces &interfaces)
{
    ...
    for (const QDBusIntrospection::Interface *interface : interfaces)
    {
        ...
        for (const QDBusIntrospection::Method &method : interface->methods)
        {
            ...
            if (parentClassName.isEmpty() && method.inputArgs.count() <= 10 && method.outputArgs.count() <= 1)
